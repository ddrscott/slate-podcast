...
export const POST: APIRoute = async (ctx) => {
  try {
    ...
    const db = getDb(ctx);
    const slot = await db.prepare('SELECT slate_id, topic_id FROM slots WHERE id = ?').bind(slotId)
      .first<{ slate_id: string; topic_id: string | null }>();
    ...
    if (action === 'publish-notes') {
      ...
      await Enqueue.notesPublished(ctx, {
        slateId: slot.slate_id, actorId: user.id, slotId, topicId: slot.topic_id,
      });
