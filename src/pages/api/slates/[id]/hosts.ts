...
// Promote a Member to Host. Allowed for any Host on the slate
// (or App Admin via the Host check's implicit pass-through). Promotion
// is additive, so peer-promote is the trust model — demotion stays
// App-Admin to avoid host-on-host coups.
//
// If the user isn't a member yet, they're inserted as a Host directly.
export const POST: APIRoute = async (ctx) => {
  try {
    ...
    const t = now();
    // Upsert: if member, promote; if absent, insert as host.
    await db.prepare(
    ...
    await Enqueue.hostPromoted(ctx, { slateId, actorId: caller.id, targetUserId: userId });

    return jsonOk({ user_id: userId, role: 'host' });
